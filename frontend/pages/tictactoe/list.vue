  ...
  <div class="">
    ...
    <main>
      ...
      <table class="w-full mt-5">
        ...
        <tbody>
          <SpecTictactoeListItem v-for="room in rooms" :room="room" :key="room.id" />
          <!-- Case empty -->
...
import type { TicTacToeGame } from "~/types/game.tictactoe";
import { ref } from "vue";

const isCreatingRoom = ref(false);

const rooms = ref<TicTacToeGame[]>([]);
function back() {
  $router.back();
}
function createRoom() {
...
function searchRoom() {}
($socket as Socket).emit(SOCKET_EVENTS.GAME.TICTACTOE.REGISTER_LIST);
($socket as Socket).on(SOCKET_EVENTS.GAME.TICTACTOE.LIST, (list: TicTacToeGame[]) => {
  rooms.value = list.filter((room) => room.id);
});

onUnmounted(() => {
  ($socket as Socket).emit(SOCKET_EVENTS.GAME.TICTACTOE.UNREGISTER_LIST);
  ($socket as Socket).off(SOCKET_EVENTS.GAME.TICTACTOE.LIST);
});
